...
| **Licence** | Apache 2.0 | MIT | MIT (OSS Mar 2026) | Apache 2.0 core + enterprise licence | Apache 2.0 |
| **GitHub stars** | ~3k | ~40k | ~11k | ~10k | ~5k |
| **Primary model** | Self-hosted binary | Self-hosted proxy | Managed SaaS + OSS | Self-hosted (Kong existing infra) | Managed SaaS + self-hosted |
...
| 🟠 Partial | Available but limited or gated |
...
|---|---|---|---|---|---|
| **Gateway latency overhead** | ✅ 11 µs mean at 5k RPS | ❌ Hundreds of µs; cascades at 2k+ RPS | 🟡 <1ms (self-reported, managed) | 🟡 Variable; NGINX-level — heavy per AI req | 🟡 ~50ms (proxy hop) |
...
| **Custom / self-hosted models** | 🟡 Custom providers supported | ✅ Ollama, vLLM, Sagemaker, NIM, HuggingFace | 🟡 Supported | 🟡 Supported as API proxy | 🟠 Limited; primarily cloud providers |
...
**Key finding:** LiteLLM and Portkey lead on provider breadth by a wide margin. Bifrost's 20+ covers the common cases (OpenAI, Anthropic, Bedrock, Vertex, Azure, Groq, Mistral, Ollama) but if you need Sagemaker, NIM, or obscure providers, LiteLLM is the only choice.
...
**Key finding:** All gateways support basic failover. HA clustering in OSS is only reliable in LiteLLM (via Redis) and Kong (native). Bifrost's OSS tier is single-replica; cluster mode is enterprise-gated — a meaningful gap for production multi-replica deployments without a PostgreSQL state backend.
...
| **RBAC** | 🟠 Enterprise-only | 🟠 Available; some features in beta | 🟡 Org/workspace isolation; RBAC | ✅ Policy-as-code; federated org-wide | 🟠 Basic; not enterprise-grade |
...
**Key finding:** For regulated / financial environments, the governance picture is bleak in OSS tiers across the board. Bifrost, LiteLLM, and Kong all gate SAML/SSO and audit logs behind enterprise tiers. Portkey has the strongest OSS guardrails coverage (50+ since March 2026 open-sourcing). Kong has the most mature enterprise governance — but requires existing Kong infrastructure.

> ⚠️ **Financial services context:** SAML, immutable audit logs, vault integration, and content guardrails are all enterprise-tier features across every gateway reviewed. Budget for enterprise licensing from day one if these are hard requirements.
...
| Feature | Bifrost | LiteLLM | Portkey | Kong AI | Helicone |
|---|---|---|---|---|---|
| **MCP gateway (client + server)** | ✅ Full client+server; /mcp endpoint; per-key tool allow-lists | 🟠 Fixed-endpoint MCP; limited security/observability | 🟡 OSS March 2026; auth, access control, identity forwarding | ✅ Enterprise MCP gateway; auto-gen from any API; OAuth | ❌ Not a MCP gateway |
...
| **Federated / OAuth MCP auth** | 🟠 Enterprise-only; OAuth 2.0 + PKCE | ❌ Not documented | 🟡 Identity forwarding (email, team, roles) | ✅ Enterprise; auto-gen MCP from any API; centralised OAuth | ❌ Not applicable |
...
**Key finding:** Portkey has the most polished semantic caching — embedded, no external vector store required. Bifrost's semantic caching is capable but requires a separately-deployed Weaviate/Qdrant instance — additional operational surface area on k3d or production k8s.
...
| **LiteLLM** | Pod + PostgreSQL + Redis (state for distributed rate limiting) |
...
| Feature | Bifrost | LiteLLM | Portkey | Kong AI | Helicone |
|---|---|---|---|---|---|
| **OSS licence** | ✅ Apache 2.0; full-featured OSS core | ✅ MIT; 40k+ GitHub stars | ✅ MIT (OSS since March 2026) | 🟠 Apache 2.0 core; enterprise licence for production | ✅ Apache 2.0; free tier 100k req/mo |
...
### Published pricing reference (April 2026)
...
> ⚠️ LiteLLM TCO note: while the licence costs $0, self-hosted production deployments (Redis + PostgreSQL + ops labour) typically run $2,000–$3,500/month — more expensive than managed alternatives at <5M requests/month.

---

## Overall Scores

Scores derived from the feature assessments above (✅ = 4, 🟡 = 3, 🟠 = 2, ❌ = 1).
...
- **Operational simplicity matters.** Single binary, SQLite default — up in minutes on k3d with a single `kubectl apply`.
...
- **Watch out for:** Severe performance ceiling (~500 RPS before degradation). Hidden TCO (Redis + PostgreSQL + ops) of $2–3.5k/mo. RBAC/SSO enterprise-gated.
...
- **Guardrails are required in OSS.** 50+ guardrail types since March 2026 open-sourcing; best-in-class for content safety without enterprise licensing.
...
- **Pricing transparency matters** — $25/mo Pro flat for unlimited requests is the clearest proposition in the market.
- **You're observability-first** and already have another layer handling routing/failover.
...
*Review compiled April 2026. Benchmark figures include vendor-published data and should be independently validated before use in procurement decisions.*
