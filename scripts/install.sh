...
# ── Detect cluster type ───────────────────────────────────────────────────────
detect_cluster_type() {
  local ctx="${CONTEXT:-$(kubectl_cmd config current-context 2>/dev/null)}"
  if [[ "$ctx" == k3d-* ]]; then
    echo "k3d"
  elif [[ "$ctx" == kind-* ]]; then
    echo "kind"
    ...
    local server
    server=$(kubectl_cmd config view --minify -o jsonpath='{.clusters[0].cluster.server}' 2>/dev/null || true)
    if [[ "$server" == *"6443"* ]]; then
      echo "k3d"
    else
      echo "kind"
...
echo "    Context:      ${CONTEXT:-$(kubectl_cmd config current-context)}"
...
run "kubectl_cmd create namespace $NS --dry-run=client -o yaml | kubectl_cmd apply -f -"

# ── Step 3: Encryption key secret ────────────────────────────────────────────
echo "--- Step 3: Encryption key secret"
if ! $DRY_RUN && kubectl_cmd -n $NS get secret bifrost-encryption-key &>/dev/null; then
  echo "    Secret already exists — skipping"
else
  run "kubectl_cmd create secret generic bifrost-encryption-key \
    --namespace $NS \
...
if [[ "$CLUSTER_TYPE" == "k3d" ]]; then
  ...
  if ! $DRY_RUN; then
    ...
    if kubectl_cmd -n $NS get endpointslice mcp-kubernetes-sse &>/dev/null; then
      kubectl_cmd -n $NS delete endpointslice mcp-kubernetes-sse && \
        echo "    Deleted stale EndpointSlice mcp-kubernetes-sse" || true
    else
      echo "    No stale EndpointSlice found — OK"
    fi
  else
    echo "[DRY-RUN] kubectl_cmd -n $NS delete endpointslice mcp-kubernetes-sse (if exists)"
  fi
fi
...
if ! $DRY_RUN; then
  if kubectl_cmd -n kube-system get deployment metrics-server &>/dev/null; then
    echo "    Metrics Server already installed — skipping"
  else
    echo "    Installing Metrics Server..."
    kubectl_cmd apply -f "$METRICS_URL"
    ...
    kubectl_cmd -n kube-system rollout status deployment/metrics-server --timeout=90s && \
      echo "    ✓ Metrics Server ready" || \
      echo "    ✗ Metrics Server not ready — pods_top may fail for a few minutes"
  fi
else
  echo "[DRY-RUN] kubectl_cmd apply -f $METRICS_URL"
  if [[ "$CLUSTER_TYPE" == "kind" ]]; then
    echo "[DRY-RUN] kubectl_cmd patch deployment metrics-server -n kube-system (--kubelet-insecure-tls)"
  fi
fi
...
if ! $DRY_RUN; then
  kubectl_cmd -n $NS rollout status statefulset/bifrost --timeout=120s

  if [[ "$CLUSTER_TYPE" == "kind" ]]; then
    echo "    Waiting for socat proxy pod..."
    kubectl_cmd -n $NS rollout status deployment/mcp-kubernetes-proxy --timeout=60s
  fi
fi
...
if ! $DRY_RUN; then
  BIFROST_POD=$(kubectl_cmd -n $NS get pod -l app=bifrost -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "bifrost-0")
  echo "    Testing from pod $BIFROST_POD..."
  if kubectl_cmd -n $NS exec "$BIFROST_POD" -- \
    ...
    if [[ "$CLUSTER_TYPE" == "kind" ]]; then
      echo "      kubectl_cmd -n $NS logs deploy/mcp-kubernetes-proxy"
    fi
  fi
fi
...
if [[ "$CLUSTER_TYPE" == "k3d" ]]; then
  echo "    Run: kubectl_cmd -n $NS port-forward svc/bifrost 8080:8080 &"
  PF_PORT=8080
else
  echo "    Run: kubectl_cmd -n $NS port-forward svc/bifrost 8080:8080 &"
...
if $DRY_RUN; then
  ...
  if [[ "$CLUSTER_TYPE" == "k3d" ]]; then
    echo "     kubectl --context ${CONTEXT:-$(kubectl_cmd config current-context)} -n $NS port-forward svc/bifrost 8080:8080 &"
  else
    echo "     kubectl --context ${CONTEXT:-$(kubectl_cmd config current-context)} -n $NS port-forward svc/bifrost 8080:8080 &"
  fi
  echo ""
